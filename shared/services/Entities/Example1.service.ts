...
import {
  CreateExample1DtoPayload,
  EditExample1DtoPayload,
} from "~/shared/models/Entities/types/example1";
import { Example1Api } from "~/shared/api/Example1.api";

export class Example1Service extends EntityServiceStrategy<
  Example1Model,
  CreateExample1DtoPayload,
  EditExample1DtoPayload
> {
  ...
  async fetchEntityRecords(
    page: number
  ): Promise<EntityPagination<Example1Model>> {
    try {
  ...
  createEntity(_payload: CreateExample1DtoPayload): Promise<void> {
    throw new Error("createEntity method not implemented yet");
  }

  updateEntity(_payload: EditExample1DtoPayload): Promise<void> {
    throw new Error("updateEntity method not implemented yet");
  }

  deleteEntity(_entityId: string): Promise<void> {
